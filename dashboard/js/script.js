...
var config = {
  ...
  var balance = firebase.database().ref().child("vehicles").child(rfid_key).child("balance");
  balance.on('value',x => balance_selector.innerHTML = x.val());
  var car_no = firebase.database().ref().child("vehicles").child(rfid_key).child("car_no");
  // TODO: check these ASAP
  // car_no.on('value',x => car_no_selector.innerHTML = x.val());
  var rfid_no = firebase.database().ref().child("vehicles").child(rfid_key).child("rfid_no");
  rfid_no.on('value',x => rfid_no_selector.innerHTML = x.val());
  var stolen = firebase.database().ref().child("vehicles").child(rfid_key).child("stolen");
  // TODO: check these ASAP
  stolen.on('value',x => {
    if(x.val()!==false){
  ...
  // user
  var email = firebase.database().ref().child("vehicles").child(rfid_key).child("user").child("email");
  email.on('value',x => email_selector.innerHTML = x.val());
  var f_name = firebase.database().ref().child("vehicles").child(rfid_key).child("user").child("name").child("first");
  f_name.on('value',x => {
  	f_name_selector.innerHTML = x.val()
  	f_name_selector_1.innerHTML = x.val()
  });
  var l_name = firebase.database().ref().child("vehicles").child(rfid_key).child("user").child("name").child("last");
  l_name.on('value',x => l_name_selector.innerHTML = x.val());
  var phone_no = firebase.database().ref().child("vehicles").child(rfid_key).child("user").child("phone_no");
  phone_no.on('value',x => phone_no_selector.innerHTML = x.val());

  // vehicle
  var vehicle_category = firebase.database().ref().child("vehicles").child(rfid_key).child("vehicle").child("vehicle_category");
  vehicle_category.on('value',x => vehicle_category_selector.innerHTML = x.val());
  var vehicle_color = firebase.database().ref().child("vehicles").child(rfid_key).child("vehicle").child("vehicle_color");
  vehicle_color.on('value',x => vehicle_color_selector.innerHTML = x.val());
  var vehicle_company = firebase.database().ref().child("vehicles").child(rfid_key).child("vehicle").child("vehicle_company");
  vehicle_company.on('value',x => {
  	vehicle_company_selector.innerHTML = x.val()
  	vehicle_company_selector_1.innerHTML = x.val()
  });
  var vehicle_image = firebase.database().ref().child("vehicles").child(rfid_key).child("vehicle").child("vehicle_image");
  ...
  var vehicle_model = firebase.database().ref().child("vehicles").child(rfid_key).child("vehicle").child("vehicle_model");
  vehicle_model.on('value',x => {
  	vehicle_model_selector.innerHTML = x.val()
  	vehicle_model_selector_1.innerHTML = x.val()
  });
  var vehicle_no = firebase.database().ref().child("vehicles").child(rfid_key).child("vehicle").child("vehicle_no");
  vehicle_no.on('value',x => {
  	vehicle_no_selector.innerHTML = x.val()
  	vehicle_no_selector_1.innerHTML = x.val()
  });
  var vehicle_type = firebase.database().ref().child("vehicles").child(rfid_key).child("vehicle").child("vehicle_type");
